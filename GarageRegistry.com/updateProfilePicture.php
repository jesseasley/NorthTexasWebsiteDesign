<?php
    ...
    // Check file size
    if ($_FILES["editProfilePicturePhoto"]["size"] > 1000000) {
        if ($json > "")
            $json .= ",";
    ...
    if ($uploadOk == 0) {
        if ($json > "")
            $json .= ",";
        $json .= "{\"message\":\"Your photo was not uploaded. (" . $_FILES["trimProfilePictureFile"] . ")\"}";

    // if everything is ok, try to upload file
    } else {
        if (compress($_FILES["editProfilePicturePhoto"]["tmp_name"], "images\\" . $target_file)) {
            if ($json > "")
                $json .= ",";
            ...
            $sql .= "ts = '" . $sd . "' ";
            $sql .= "where id = '" . $_POST["editProfilePictureUserID"] . "'";
